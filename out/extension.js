...
function normalizeMacro(raw) {
  ...
  const steps = data.steps.map((step) => normalizeStep(step)).filter(Boolean);
  if (!steps.length) {
    return undefined;
  }
  return {
...
function parseSteps(raw) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      const steps = parsed.map((item) => normalizeStep(item)).filter(Boolean);
      return steps;
    }
  } catch (err) {
    // ignore
...
async function promptForStep(existing) {
  const find = await vscode.window.showInputBox({
    prompt: '検索文字列 (find)',
    value: (existing === null || existing === void 0 ? void 0 : existing.find) ?? '',
    ignoreFocusOut: true
  });
  if (find === undefined) {
    return undefined;
  ...
  const replace = await vscode.window.showInputBox({
    ...
    value: (existing === null || existing === void 0 ? void 0 : existing.replace) ?? '',
    ignoreFocusOut: true
  });
  if (replace === undefined) {
    return undefined;
  ...
  const useRegexPick = await vscode.window.showQuickPick(
    [
      { label: '通常の文字列として検索', value: false, picked: !(existing === null || existing === void 0 ? void 0 : existing.useRegex) },
      { label: '正規表現として検索', value: true, picked: !!(existing === null || existing === void 0 ? void 0 : existing.useRegex) }
    ],
    { placeHolder: '検索方法を選択' }
  );
  if (!useRegexPick) {
  ...
  const casePick = await vscode.window.showQuickPick(
    [
      { label: '大文字小文字を区別する', value: true, picked: !!(existing === null || existing === void 0 ? void 0 : existing.caseSensitive) },
      { label: '大文字小文字を区別しない', value: false, picked: !(existing === null || existing === void 0 ? void 0 : existing.caseSensitive) }
    ],
  ...
  const interpretPick = await vscode.window.showQuickPick(
    [
      { label: '置換後の \\n, \\t を解釈する', value: true, picked: (existing === null || existing === void 0 ? void 0 : existing.interpretEscapes) !== false },
      { label: '解釈しない（そのまま文字列を挿入）', value: false, picked: (existing === null || existing === void 0 ? void 0 : existing.interpretEscapes) === false }
    ],
  ...
  const note = await vscode.window.showInputBox({
    prompt: 'メモ (任意)',
    value: (existing === null || existing === void 0 ? void 0 : existing.note) ?? '',
    ignoreFocusOut: true,
...
class MacroTreeItem extends vscode.TreeItem {
  constructor(macro) {
    ...
    this.tooltip = macro.steps
      .map(
        (step, i) =>
          `${i + 1}. ${step.useRegex ? '/' : ''}${step.find}${step.useRegex ? '/' : ''} -> ${step.replace}${
            step.note ? ` (${step.note})` : ''
          }`
      )
      .join('\n');
